...
class TwoLayerNet(object):
	...
	def loss(self, X, y=None, reg=0.0):
		...
		correct_logprops = -np.log(prob[np.arange(N), y])

		data_loss = np.sum(correct_logprops) / N
	...
	def predict(self, X):
		...
		###########################################################################
		relu = lambda x: np.maximum(0, x)
		softmax = lambda x: 1.0 / (1.0 + np.exp(-x))

		hidden1 = relu(np.dot(X, self.params['W1']) + self.params['b1'])
		out = np.dot(hidden1, self.params['W2']) + self.params['b2']
